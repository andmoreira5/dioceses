module.exports.data = [
    {
        titulo:'Presidência Estadual',
        content:[
            {
                nome:'Aurilene Lavor',
                funcao:'Presidente Estadual'
            },
            {
                nome:'Pe. Helano',
                funcao:'Diretor Espiritual'
            },
            {
                nome:'Tassara Sarmento',
                funcao:'Secretária Geral'
            },
            {
                nome:'Sheridan Mota',
                funcao:'1ª Secretária e Tesoureira'
            },
            {
                nome:'Dales Lopes',
                funcao:'2º Secretário'
            }
           
        ]
    },
    {
        titulo:'Coordenadores Diocesanos',
        content:[
            {
                nome:'Luana Gabrielly',
                funcao:'Crato'
            },
            {
                nome:'Tiago Marques',
                funcao:'Iguatu'
            },
            {
                nome:'Tatiane Leal',
                funcao:'Quixadá'
            },
            {
                nome:'José Moura',
                funcao:'Itapipoca'
            },
            {
                nome:'Robério Almeida',
                funcao:'Tianguá'
            },
            {
                nome:'Romário Silva',
                funcao:'Limoeiro do Norte'
            },
            {
                nome:'Francisca Dalba',
                funcao:'Crateús'
            },
            {
                nome:'Vanderlúcia Menezes',
                funcao:'Sobral'
            },
            {
                nome:'Vera Cruz',
                funcao:'Fortaleza'
            }
        ]
    },
    {
        titulo:'Coordenadores de Ministério',
        content:[
            {
                nome:'Marcos Farias',
                funcao:'Comunicação Social'
            },
            {
                nome:'Germana Carvalho',
                funcao:'Crianças e Adolescentes'
            },
            {
                nome:'Paulo Matos',
                funcao:'Cura e Libertação'
            },
            {
                nome:'Pedro e Rosângela',
                funcao:'Família'
            },
            {
                nome:'Abenaide de Sousa',
                funcao:'Fé e Política'
            },
            {
                nome:'Roger Araújo',
                funcao:'Formação'
            },
            {
                nome:'Carlos Alberto',
                funcao:'Intercessão'
            },
            {
                nome:'Cícero Bruno',
                funcao:'Jovem'
            },
            {
                nome:'Lorena Bessa',
                funcao:'Música e Artes'
            },
            {
                nome:'Eclésia Xavier',
                funcao:'Pregação'
            },
            {
                nome:'Rosines Brito',
                funcao:'Promoção Humana'
            },
            {
                nome:'Ir. Catarina',
                funcao:'Religiosas'
            },
            {
                nome:'Dyrles Marques',
                funcao:'Seminaristas'
            },
            {
                nome:'Ivan Nascimento',
                funcao:'Universidades Renovadas'
            }
        ]
    },
]