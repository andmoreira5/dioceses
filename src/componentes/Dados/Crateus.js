module.exports.data = [
    {
        titulo:'Presidência Diocesana',
        content:[
            {
                nome:'Dalba Cardoso',
                detalhes:'Presidente'
            },
            {
                nome:'Erlania Pontes',
                detalhes:'Secretária Geral'
            }
        ]
    },
    {
        titulo:'Coordenadores de Grupos de Oração',
        content:[
            {
                nome:'Márcia Siriano',
                detalhes:'Adoradores de Cristo (Novo Oriente)'
            },
            {
                nome:'Maria Bernardes',
                detalhes:'Fonte de Água Viva (Paramburu)'
            },
            {
                nome:'José Victor',
                detalhes:'Louvor e Adoração (Tauá)'
            },
            {
                nome:'Fátima Aguiar',
                detalhes:'Nova aliança (Independência)'
            },
            {
                nome:'Aulixiadora Gomes',
                detalhes:'O Amor Vencerá (Crateús)'
            },
            {
                nome:'Marcos Evangelista',
                detalhes:'Rainha da Paz (Crateús)'
            },
            {
                nome:'Matheus Farias',
                detalhes:'Restauração (Nova Russas)'
            },
        ]
    },
    {
        titulo:'Coordenadores de Ministério',
        content:[
            {
                nome:'Vinicius Costa',
                detalhes:'Comunicação Social'
            },
            {
                nome:'Francisco Afranio',
                detalhes:'Intercessão'
            },
            {
                nome:'Rosana Patrícia',
                detalhes:'Música e Artes'
            },
            {
                nome:'Irisnalda Lima',
                detalhes:'Pregação'
            }
        ]
    },
]